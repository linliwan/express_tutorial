...
import { container, BlogByTag } from "./models.ts"
...
async function fetchAndRenderBlogsByTag(tag: string, limit: number = 10, offset: number = 0) {
    try {
...
function renderPagination(tag: string, total: number, limit: number, offset: number) {
